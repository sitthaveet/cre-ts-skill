...
const postData = (
  ...
  const req = {
    ...
    cacheSettings: {
      store: true, // Use 'store', not 'readFromCache'
      maxAge: { seconds: 60n }, // Use 'maxAge' (Duration type), not 'maxAgeMs'
    },
  };
...
const onCronTrigger = (runtime: Runtime<Config>): string => {
  ...
  const result = httpClient
    ...
    )(runtime.config) // IMPORTANT: sendRequest returns a function that must be called with arguments
    .result();
